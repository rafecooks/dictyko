...
3/17/15 Patched bug in restriction enzyme selection
...
parser = argparse.ArgumentParser(
    description='Automate pLPBLP knockout vectors for Dictyostelium')
parser.add_argument(
    'genes', help='Enter single gene name or .txt file with list of names')
parser.add_argument(
    '--email', '-e', required=True, help='Required to query NCBI databases.')
...
def fetch_gene_coordinates(gene):
    ...
    """
    fetch_log = "Fetching_gene_coordinates for {}\n".format(gene)
    log.write(fetch_log)
    ...
    ## In the future, write a bit to determine number of records,
    ## then figure out which is the correct record
    recnum = len(rec["Entrezgene_locus"]) - 1
    gene_locus = rec["Entrezgene_locus"][recnum]
        ...
        for feat in record.features:  # we need to deal with instances for predicted genes
            if (feat.type == "gene"):  # that do not have a "gene" feature qualifier,
                ...
                if (gene in feat.qualifiers["gene"]) or (gene in feat.qualifiers["locus_tag"]):
                    check = check + 1
        if (check > 0):
            SeqIO.write(record, genedir + "/" + outfile, "genbank")
            confirm = "Writing genbank files for: {}\n".format(gene)
            log.write(confirm)
            print confirm
            return outfile
        else:
            error = "Error: Could not accurately identify gene {}\n".format(gene)
            print error
            log.write(error)
            return "error"
...
def runp3(geneHA, sequence, HApos, HA):
    ...
    if (p3output['PRIMER_PAIR_NUM_RETURNED'] < 1):
        print "Failed to find primers for {}\n".format(geneHA)
        log.write("Failed to find primers for {}\n".format(geneHA))
        return "error"
    else:
        return p3output
...
def parsep3(p3output, geneHA):
    ...
    ##append output table
    with open(gspcsv, 'a') as pf:
        # print "writing primers for ", geneHA
        log.write("writing primers for {}\n".format(geneHA))
        pf.write(geneHA + ",")
        pf.write(Lprimer + ",")
        pf.write(str(Ltm) + ",")
        pf.write(Lstart + ",")
...
def HA_REs(HA5str, HA3str):
    ...
    typeIIs = [BbsI, BsaI]
    absentTypeII = []
    absentA5, absentA3, absentB5, absentB3 = [], [], [], []
    absAboth, absBboth = [], []
    # initialize markers
    A5_B3 = 0
    B5_A3 = 0
    ...
    for enz in mcsB:
        ...
        # first look if there are at least 2 sites per MCS with no conflict
    if ((len(absAboth) > 1) and (len(absBboth) > 1)):
        A5_B3 = 1
        addRE55 = absAboth[0]
        addRE53 = absAboth[1]
        addRE33 = absBboth[0]
    ...
    # next: at least 2 sites in mcsA and match with mcsB
    elif ((len(absAboth) > 1) and (len(absentB3) > 0)):
        A5_B3 = 1
        addRE55 = absAboth[0]
        addRE53 = absAboth[1]
        ...
        # next: at least 2 sites in mcsB and match with mcsA, reverse orientation
    elif ((len(absBboth) > 1) and (len(absentA3) > 0)):
        B5_A3 = 1
        addRE53 = absBboth[0]
        addRE55 = absBboth[1]
        ...
        # next: back to previous approach
    elif ((len(absentA5) > 0) and (len(absentB3) > 0)):
        A5_B3 = 1
        if (len(absentA5) > 1):
            addRE55 = absentA5[0]
            addRE53 = absentA5[1]
    ...
    if A5_B3 == 1:
        HAorder = "A5_B3"
        if (len(addRE33.search(HA5)) > 0) or (len(addRE35.search(HA5)) > 0):
            digest_order = 2
        else:
            digest_order = 1
    ...
    return error, addRE55, addRE53, addRE35, addRE33, typeII, HAorder, digest_order


# subroutine to take subsection of GB file and
# maintain partial annotations (that would be automatically lost)
def GBslice(Record, HAseq):
    slice_log = "GBslice for {}\n".format(gene)
    log.write(slice_log)
    fullseq = Record.seq
    ...
    for feature in Record.features:
        if (sl_st in feature) and (feature.type == "gene"):
            ...
            slicefeature1 = SeqFeature(slicefeature1_loc, type=slicefeature1_type, strand=slicefeature1_strand,
                                       qualifiers=slicefeature1_qualifier)
        ...
        if (sl_end in feature) and (feature.type == "gene"):
            ...
            slicefeature2 = SeqFeature(slicefeature2_loc, type=slicefeature2_type, strand=slicefeature2_strand,
                                       qualifiers=slicefeature2_qualifier)
...
def addREprimers(frag, RE5, RE3, Lleng, Rleng, typeII, HAorder, HAnum):
    addREprime_log = "addREprimers for {}\n".format(gene)
    ...
    label2F = "HA" + str(HAnum) + "_primerFwd"
    label2R = "HA" + str(HAnum) + "_primerRev"
    Fprime_qual = {'label': label2F}
    Rprime_qual = {'label': label2R}
...
def pLPBLP_merge(HA5, HA3, RE55, RE53, RE35, RE33, HAorder, digest_order):
    merge_log = "pLPBLP_merge for {}\n".format(gene)
    log.write(merge_log)
    pLPBLP = SeqIO.read("pLPBLP.gb", "genbank")

    # print (RE55, RE53, RE35, RE33, HAorder, digest_order)
    ...
    if HAorder == "A5_B3":
        ...
        LPB5 = (str(pLPBLP.seq)).find(RE35.site)
        LPB3 = (str(pLPBLP.seq)).find(RE33.site)
    ...
    # same as above, except use the RE3 sites instead of RE5 sites to set the mcsA digestion
        ...
        LPB5 = (str(pLPBLP.seq)).find(RE55.site)
        LPB3 = (str(pLPBLP.seq)).find(RE53.site)

    # now deal with which mcs gets cut first
    if (digest_order == 1):
        mcsA_remove = (6 + LPA3) - LPA5
        pLPBLP_shiftA = pLPBLP[LPA5:] + pLPBLP[:LPA5]
        mcsA_digest = pLPBLP_shiftA[mcsA_remove:]
        ...
        mcsB_remove = (6 + LPB3) - LPB5
        concat_shiftB = concatA[LPB5:] + concatA[:LPB5]
        mcsB_digest = concat_shiftB[mcsB_remove:]
        merged_map = B_HA[3:(len(B_HA) - 3)] + mcsB_digest

    elif (digest_order == 2):
        mcsB_remove = (6 + LPB3) - LPB5
        pLPBLP_shiftB = pLPBLP[LPB5:] + pLPBLP[:LPB5]
        mcsB_digest = pLPBLP_shiftB[mcsB_remove:]
        concatB = B_HA[3:(len(B_HA) - 3)] + mcsB_digest

        # add arm into mcsA
        if HAorder == "A5_B3":
            LPA5 = (str(concatB.seq)).find(RE55.site)
            LPA3 = (str(concatB.seq)).find(RE53.site)
        if HAorder == "B5_A3":
            LPA5 = (str(concatB.seq)).find(RE35.site)
            LPA3 = (str(concatB.seq)).find(RE33.site)

        mcsA_remove = (6 + LPA3) - LPA5
        concat_shiftA = concatB[LPA5:] + concatB[:LPA5]
        mcsA_digest = concat_shiftA[mcsA_remove:]
        merged_map = A_HA[3:(len(A_HA) - 3)] + mcsA_digest
...
def GBrc(record):
    """
    subroutine to generate annotated reverse complement
    """
    rc_seq = record.seq.reverse_complement()
    rc_record = SeqRecord(rc_seq, id=record.id, name=((record.name) + "_rc"))

    for feature in record.features:

            exact_start = feature.location.nofuzzy_start
            exact_end = feature.location.nofuzzy_end
            rc_start = len(rc_seq) - exact_end
            rc_end = len(rc_seq) - exact_start
            rc_location = FeatureLocation(rc_start, rc_end)
            #rc_qualifier = {'gene': feature.qualifiers["gene"]}
            #not sure if I need to do more to get all qualifiers
            rc_strand = (feature.strand) * (-1)
            rc_feature = SeqFeature(rc_location, type=feature.type, strand=rc_strand, qualifiers=feature.qualifiers)
            rc_record.features.append(rc_feature)

    return rc_record


def build_final_map(locus, KO, Fseq5, Rseq5, Fleng5, Fseq3, Rseq3, Rleng3, HAorder):
    ...
    if HAorder == "A5_B3":
        ...
        inStart = (KOshift.seq).find(str(Fseq5))
        ...
        KOshift = KO[shiftpos:] + KO[:shiftpos]
        inStart = (KOshift.seq).find(str(Rseq3))
        inEnd = (KOshift.seq).find(Fseq5rc) + int(Fleng5)
        insert = KOshift[inStart:inEnd]
        rc_insert = GBrc(insert)
        finalmap = leftRec + rc_insert + rightRec
...
timedir = "{}_dictyko_run".format(timestamp)
os.mkdir(timedir)

logfile = "{}/log.txt".format(timedir)
log = open(logfile, "w")
##initiate output file

gspcsv = "{}/gene_specific_primers.csv".format(timedir)
with open(gspcsv, 'w') as pf:
    pf.write('GeneHA,Left_seq,Left_tm,Left_start,Left_length,Right_seq,Right_tm,Right_start,Right_len,HA_len\n')

p2ocsv = "{}/primers_to_order.csv".format(timedir)
p2o = open(p2ocsv, 'w')
...
for gene in genelist:
    ...
    genedir = "{}/results_{}".format(timedir, gene)
    os.mkdir(genedir)
    ...
    HA5_Lstart, HA5_Lleng, HA5_Rstart, HA5_Rleng, fwd5TM, rev5TM, Fseq5, Rseq5 = parsep3(HA5_primers,
                                                                                         geneHA5)  #parse relevant info

    HA3_primers = runp3(geneHA3, sequence, HA3pos, 3)  #get primers for HA3
    if HA3_primers == "error":
        continue

    HA3_Lstart, HA3_Lleng, HA3_Rstart, HA3_Rleng, fwd3TM, rev3TM, Fseq3, Rseq3 = parsep3(HA3_primers,
                                                                                         geneHA3)  #parse relevant info
    ...
    REerror, addRE55, addRE53, addRE35, addRE33, typeII, HAorder, digest_order = HA_REs(HA5_seq,
                                                                                        HA3_seq)  #check for available RE sites in HAs
    if REerror == 1:
        continue

    ##Build GB files for homology arms
    HA5_fragment = GBslice(seqRecord, HA5_seq)  #get annotated slice of seqRecord
    HA5_record, fwd5, rev5 = addREprimers(HA5_fragment, addRE55, addRE53, HA5_Lleng, HA5_Rleng, typeII, HAorder,
                                          5)  #build HA amplicon GB
    HA5_record.name = gene + "_HA5"
    SeqIO.write(HA5_record, genedir + "/" + gene + "_HA5.gb", "genbank")
    HA3_fragment = GBslice(seqRecord, HA3_seq)  #get annotated slice of seqRecord
    HA3_record, fwd3, rev3 = addREprimers(HA3_fragment, addRE35, addRE33, HA3_Lleng, HA3_Rleng, typeII, HAorder,
                                          3)  #build HA amplicon GB
    HA3_record.name = gene + "_HA3"
    SeqIO.write(HA3_record, genedir + "/" + gene + "_HA3.gb", "genbank")

    ##Merge pLPBLP vector map with HA maps
    pLPBLP_HAs = pLPBLP_merge(HA5_record, HA3_record, addRE55, addRE53, addRE35, addRE33, HAorder, digest_order)
    pNAME = gene + "_pLPBLP.gb"
    pLPBLP_HAs.name = gene + "_KO"
    SeqIO.write(pLPBLP_HAs, genedir + "/" + pNAME, "genbank")

    ##Construct and write final insert map
    #insert_map = seqRecord[:(int(HA5_Rstart)+1)] + BsR_insert + seqRecord[int(HA3_Lstart):]
    final_map = build_final_map(seqRecord, pLPBLP_HAs, Fseq5, Rseq5, HA5_Lleng, Fseq3, Rseq3, HA3_Rleng, HAorder)
    mapname = gene + "_locus_BsR_insertion.gb"
    SeqIO.write(final_map, genedir + "/" + mapname, "genbank")

    ##print primers_to_order
    ha5f = str(gene + '_HA5_fwd,' + fwd5 + ',' + str(fwd5TM) + ',' + str(addRE55) + ',' + str(typeII) + ',' + str(
        len(HA5_record)) + ' bp\n')
    ha5r = str(gene + '_HA5_rev,' + rev5 + ',' + str(rev5TM) + ',' + str(addRE53) + ',,\n')
    ha3f = str(
        gene + '_HA3_fwd,' + fwd3 + ',' + str(fwd3TM) + ',' + str(addRE35) + ',,' + str(len(HA3_record)) + ' bp\n')
